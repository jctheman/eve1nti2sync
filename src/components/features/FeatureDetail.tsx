import { MessageCircle, BarChart3, Lightbulb, FileText, Calendar, CheckCircle2 } from "lucide-react";
...
const features = [{
  ...
  benefits: ["Handles email and chat queries in real-time", "Updates registration and housing information instantly", "Reduces customer service costs by up to $200K annually", "Provides 24/7 support for attendees, exhibitors, and speakers", "Can be used internally for sales and customer service"],
  ...
  benefits: ["Scores attendees from 1 to 10 based on engagement levels", "Identifies high-value prospects for exhibitors", "Increases conversion rates and exhibitor ROI", "Uses registration and behavioral data for accurate scoring", "Provides real-time lead quality insights"],
  ...
  benefits: ["Increases attendee engagement and satisfaction", "Suggests relevant sessions and exhibitors to attendees", "Makes real-time suggestions during registration", "Helps attendees discover content they might otherwise miss", "Creates personalized event journeys"],
  ...
  benefits: ["Generates welcome emails and session descriptions", "Ensures consistency in communications", "Saves time on content creation", "Creates engaging, professional content", "Can be used for marketing materials and website content"],
  ...
  benefits: ["Automates setup of online registration sites", "Provides session scheduling options", "Reduces setup time and errors", "Uses predefined business rules for consistency", "Streamlines the planning process"],
...
const FeatureDetail = () => {
  return <section className="py-12">
      <div className="container-custom">
        {features.map((feature, index) => <div key={index} id={feature.id} className={`grid grid-cols-1 lg:grid-cols-2 gap-12 py-16 ${index % 2 === 1 ? 'lg:grid-flow-dense' : ''}`}>
